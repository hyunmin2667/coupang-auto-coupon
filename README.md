...
이 섹션은 이 프로젝트의 기술적인 부분에 관심 있는 개발자분들을 위한 내용입니다.

### 기술 스택

* **Python 3.x**: 프로그램의 기본 언어입니다.
* **pandas**: CSV 파일에서 품목 데이터를 효율적으로 다룹니다.
* **selenium**: 웹 자동화를 위한 라이브러리로, 필요에 따라 쿠팡 WING 로그인 및 쿠폰 비활성화에 사용될 수 있습니다. (현재 `main.py`는 API 기반 비활성화를 우선합니다.)
* **webdriver-manager**: Selenium 웹드라이버를 자동으로 설치하고 관리합니다.
* **schedule**: 스크립트의 반복 실행 일정을 관리합니다.
* **python-dotenv/dotenv**: `.env` 파일에서 환경 변수를 안전하게 불러옵니다.
* **requests**: Discord 웹훅으로 알림을 보내는 데 사용됩니다.
* **hmac, hashlib, urllib.request, urllib.parse**: 쿠팡 API 인증 및 요청 처리에 사용되는 Python 표준 라이브러리입니다.
